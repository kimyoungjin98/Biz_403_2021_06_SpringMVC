...
public class StudentServiceImplV1 implements StudentService {
	...
	public String makeStNum() {
		...
		log.debug("현재 년도 {}, 생성된 학번 {}", curYear);
	...
	public String makeStNum(String prefix) {
		 ...
		 * stSeq = stNum.substring(4) 이런 형식의 코드가 생성되어 stSeq에는 0010 문자열만 남게된다
		 */
...
//		stVO.setSt_num(newStNum);
	...
	/*
	 * Transaction의 조건 다수의 CRUD는 한개의 업무 프로세스다 다수의 CRUD가 모두 정상적으로 완료되어야만 업무가 정상으로
	 * 수행된다
	 * 
	 * 업무가 수행되는 동안 한곳이라도 CRUD에서 오류가 발생하면 그 중 CUD가 진행되는 동안 문제가 발생하고 데이터에 오류가 저장될 것이다
	 * 
	 * 이런 상황을 방지하기 위하여 업무 단위를 Transaction이라는 단위로 묶고
	 * 
	 * 모든 업무가 완료되면 데이터를 Commit(실제저장)하고 그렇지 않으면 Rollback ALL(모두 취소하는 처리)
	 * 
	 * @Transactional로 선언된 method에서 모든 데이터를 insertOrUpdate를 수행한다음 강제로 exception을
	 * 발생하였다
	 * 
	 * 그랬더니 transactionManager에 의해서 모든 Insert Or Update가 RollBack되어 버렸다
	 */
	@Transactional
	@Override
	public String scoreInput(ScoreInputVO scInputVO) {
		...
		for (int i = 0; i < size; i++) {
			scDao.insertOrUpdate(
					...
					scInputVO.getSt_num(), scInputVO.getSubject().get(i), scInputVO.getScore().get(i)
