...
public class NaverNewsServiceImplV2 extends NaverNewsServiceImplV1{
	...
	public List<NewsDTO> getNaverList(String queryURL) throws Exception {
		...
		resList = restTemp.exchange(resURI, // naver에 요청할 URL 정보
									HttpMethod.GET,  // GET method로 요청
									entity,  // 인증정보 등이 담긴 header
									NaverParentDTO.class); // 내가 받을 데이터
		log.debug("업데이트 날짜 {} " ,resList.getBody().getLastBuildDate());
		log.debug("전체 개수 {} " ,resList.getBody().getTotal());
		log.debug("리스트 {} " ,resList.getBody().getItems().toString());
		List<NewsDTO> newsList = resList.getBody().getItems();
